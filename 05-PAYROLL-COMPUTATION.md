 ...
 */
function calculateHolidayPay(dailyRate, holidayType, worked = false, hoursWorked = 0) {
  if (holidayType === 'regular') {
    if (!worked) {
      ...
      const overtimePay = (dailyRate / 8) * overtimeHours * 0.30
    ...
    if (!worked) {
      ...
      const baseHolidayPay = dailyRate * 1.30
      const overtimeHours = Math.max(0, hoursWorked - 8)
      const overtimePay = (dailyRate / 8) * overtimeHours * 0.30
      return baseHolidayPay + overtimePay
    }
 ...
 */
function calculateNightDifferential(hourlyRate, nightHours, rate = 0.10) {
...
function calculateGrossPay(components) {
  ...
  return basicPay + overtimePay + holidayPay + nightDifferential + allowances + bonuses
}
```
...
```javascript
const sssTable = [
  { minRange: 0, maxRange: 4249.99, employeeShare: 180.00 },
  { minRange: 4250, maxRange: 4749.99, employeeShare: 202.50 },
  { minRange: 4750, maxRange: 5249.99, employeeShare: 225.00 },
  { minRange: 5250, maxRange: 5749.99, employeeShare: 247.50 },
  { minRange: 5750, maxRange: 6249.99, employeeShare: 270.00 },
  { minRange: 6250, maxRange: 6749.99, employeeShare: 292.50 },
  { minRange: 6750, maxRange: 7249.99, employeeShare: 315.00 },
  { minRange: 7250, maxRange: 7749.99, employeeShare: 337.50 },
  { minRange: 7750, maxRange: 8249.99, employeeShare: 360.00 },
  { minRange: 8250, maxRange: 8749.99, employeeShare: 382.50 },
  { minRange: 8750, maxRange: 9249.99, employeeShare: 405.00 },
  { minRange: 9250, maxRange: 9749.99, employeeShare: 427.50 },
  { minRange: 9750, maxRange: 10249.99, employeeShare: 450.00 },
  { minRange: 10250, maxRange: 10749.99, employeeShare: 472.50 },
  { minRange: 10750, maxRange: 11249.99, employeeShare: 495.00 },
  { minRange: 11250, maxRange: 11749.99, employeeShare: 517.50 },
  { minRange: 11750, maxRange: 12249.99, employeeShare: 540.00 },
  { minRange: 12250, maxRange: 12749.99, employeeShare: 562.50 },
  { minRange: 12750, maxRange: 13249.99, employeeShare: 585.00 },
  { minRange: 13250, maxRange: 13749.99, employeeShare: 607.50 },
  { minRange: 13750, maxRange: 14249.99, employeeShare: 630.00 },
  { minRange: 14250, maxRange: 14749.99, employeeShare: 652.50 },
  { minRange: 14750, maxRange: 15249.99, employeeShare: 675.00 },
  { minRange: 15250, maxRange: 15749.99, employeeShare: 697.50 },
  { minRange: 15750, maxRange: 16249.99, employeeShare: 720.00 },
  { minRange: 16250, maxRange: 16749.99, employeeShare: 742.50 },
  { minRange: 16750, maxRange: 17249.99, employeeShare: 765.00 },
  { minRange: 17250, maxRange: 17749.99, employeeShare: 787.50 },
  { minRange: 17750, maxRange: 18249.99, employeeShare: 810.00 },
  { minRange: 18250, maxRange: 18749.99, employeeShare: 832.50 },
  { minRange: 18750, maxRange: 19249.99, employeeShare: 855.00 },
  { minRange: 19250, maxRange: 19749.99, employeeShare: 877.50 },
  { minRange: 19750, maxRange: 99999.99, employeeShare: 900.00 }
]
...
function calculateSSS(monthlyCompensation) {
  const bracket = sssTable.find(
    row => monthlyCompensation >= row.minRange && monthlyCompensation <= row.maxRange
  )
  
  return bracket ? bracket.employeeShare : 900.00 // Max contribution
}
...
const taxTableSemiMonthly = [
  ...
  { minRange: 10417, maxRange: 16666, baseTax: 0, excessRate: 0.15, excessOver: 10416 },
  { minRange: 16667, maxRange: 33332, baseTax: 937.50, excessRate: 0.20, excessOver: 16666 },
  { minRange: 33333, maxRange: 83332, baseTax: 4270.83, excessRate: 0.25, excessOver: 33332 },
  { minRange: 83333, maxRange: 333332, baseTax: 16770.83, excessRate: 0.30, excessOver: 83332 },
  { minRange: 333333, maxRange: Infinity, baseTax: 91770.83, excessRate: 0.35, excessOver: 333332 }
]
...
function calculateLoanDeductions(loans, payrollItemId) {
  ...
  for (const loan of loans) {
    if (loan.status === 'active' && loan.payments_made < loan.number_of_payments) {
 ...
 */
function computePayroll(employee, attendance, allowances, bonuses, deductions, loans) {
  ...
  const overtimeHours = attendance.reduce((sum, rec) => sum + (rec.overtime_hours || 0), 0)
  ...
  let holidayPay = 0
  attendance.filter(rec => rec.is_holiday).forEach(rec => {
  ...
  const nightHours = attendance.reduce((sum, rec) => sum + (rec.night_differential_hours || 0), 0)
  ...
  // 7. Calculate bonuses
  const bonusAmounts = {
    taxable: bonuses.filter(b => b.is_taxable).reduce((sum, b) => sum + b.amount, 0),
    nonTaxable: bonuses.filter(b => !b.is_taxable).reduce((sum, b) => sum + b.amount, 0),
