  ...
  datasets: [
    {
      ...
      backgroundColor: distributionData.value.map(d => statusColors[d.label] || 'rgba(158, 158, 158, 0.8)'),
      borderColor: distributionData.value.map(d => (statusColors[d.label] || 'rgba(158, 158, 158, 0.8)').replace('0.8', '1')),
      borderWidth: 2,
      borderRadius: 8
    }
  ]
...
const loadData = async () => {
  try {
    ...
    console.error('Error loading employment status distribution:', error);
