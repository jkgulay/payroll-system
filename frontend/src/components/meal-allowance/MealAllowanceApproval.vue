    ...
    <v-card v-if="mealAllowance">
      ...
      <v-card-text class="pt-4">
        <v-alert type="info" class="mb-4">
          <strong>{{ mealAllowance.reference_number }}</strong><br>
          {{ mealAllowance.title }}
        </v-alert>
...
async function approve() {
  ...
  try {
    // First approve the meal allowance
    ...
    alert('Meal allowance approved and PDF generated successfully')
    emit('approved')
    close()
  } catch (error) {
    console.error('Error approving meal allowance:', error)
    alert('Failed to approve meal allowance')
  } finally {
...
async function reject() {
  if (!confirm('Are you sure you want to reject this meal allowance?')) return
  ...
  try {
    ...
    alert('Meal allowance rejected')
    emit('approved')
    close()
  } catch (error) {
    console.error('Error rejecting meal allowance:', error)
    alert('Failed to reject meal allowance')
  } finally {
    processing.value = false
  }
}
