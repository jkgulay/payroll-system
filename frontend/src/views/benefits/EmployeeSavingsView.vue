  ...
  <div class="cash-bond-page">
    <div class="modern-card">
      ...
      <div class="page-header">
        ...
        <button
          v-if="userRole !== 'employee' && hasAccess"
            ...
            <v-row>
              ...
              <v-col cols="12" v-if="!editMode">
                ...
                <v-btn-toggle
                  ...
                  divided
                >
                  <v-btn value="individual" size="small">Individual</v-btn>
                  <v-btn value="multiple" size="small">Multiple</v-btn>
                  <v-btn value="department" size="small">By Project</v-btn>
                  <v-btn value="position" size="small">By Position</v-btn>
                </v-btn-toggle>
              </v-col>
              ...
              >
                <v-alert type="info" variant="tonal" density="compact">
                  <strong>{{ form.employee_ids.length }}</strong> employee(s)
                  selected.
                </v-alert>
              </v-col>
        ...
        <v-card-text class="pt-4">
          ...
          <v-form ref="withdrawFormRef" v-model="withdrawFormValid">
            <v-text-field
              ...
              class="mb-2"
              hint="Can only withdraw from already deducted contributions"
              persistent-hint
            ></v-text-field>
...
const fetchDepartments = async () => {
  ...
  try {
    const response = await api.get("/deductions/departments/list");
...
const fetchPositions = async () => {
  ...
  try {
    const response = await api.get("/deductions/positions/list");
    positions.value = response.data || [];
  } catch (error) {
    devLog.error("Failed to fetch positions:", error);
  } finally {
...
const loadEmployeesByFilter = async () => {
  try {
    const payload = {};
    if (selectionMode.value === "department" && form.value.department) {
      payload.department = form.value.department;
    } else if (selectionMode.value === "position" && form.value.position) {
    ...
    const response = await api.post("/deductions/employees/filter", payload);
    affectedEmployeesCount.value = response.data?.count || 0;
  } catch (error) {
    devLog.error("Failed to load affected employees:", error);
    affectedEmployeesCount.value = 0;
  }
};
...
const openWithdrawDialog = (bond) => {
  ...
  Object.assign(withdrawForm.value, {
    ...
    withdraw_amount: contributedAmount,
    withdraw_date: new Date().toISOString().substr(0, 10),
    withdraw_reason: "",
  });
...
const saveSavingsRecord = async () => {
  ...
  if (!editMode.value) {
    if (selectionMode.value === "individual" && !form.value.employee_id) {
      showSnackbar("Please select an employee", "error");
      return;
    }
    if (
      selectionMode.value === "multiple" &&
      (!Array.isArray(form.value.employee_ids) ||
    ...
    if (selectionMode.value === "position" && !form.value.position) {
        ...
        const bulkPayload = {
          ...
          employee_ids:
            selectionMode.value === "multiple"
              ...
              : undefined,
          department:
            selectionMode.value === "department"
